"""
Implementation of the CMA-ES optimisation.  
Documentation for the cma package is at
https://pypi.org/project/cma/
The package can be installed with pip.
The options of the package are currently set with 
a dictionary, see the CMAOptions package for details.

It is implicitly assumed that the parameters to be 
optimized in the function have been scaled properly to be
of the order of 1.
"""
...
"""
Initializes the algorithm.  The options dictionary is passed to the CMAOptions object
"""
    ...
    def __call__(self, function):
        ...
        if self.es is None:
            # Set up the sampler.  Use the center of the range as initial position
            # and a third of the range as the initial sigma to cover nearly the entire 
            # parameter space initially.
            # Unbounded parameters are an issue, sample those from -3 to 3
            # This implicitly assumes the best fit value is close to 0 for the unbounded parameters
            r = np.array(ranges)
            r[:,0][np.isinf(r[:,0])] = -3
            r[:,1][np.isinf(r[:,1])] = 3
            x0 = 0.5*(r[:,1]+r[:,0])

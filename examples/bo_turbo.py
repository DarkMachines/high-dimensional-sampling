...
class BO_Turbo(hds.Procedure):
	def __init__(self, max_evals = 1000, trust_regions = 5, max_cholesky_size = 2000, n_training_steps=50, batch_size = 10):
	...
	def __call__(self, function):
		...
		turbo_m = TurboM(
    			...
    			lb=np.array(self.bounds[:, 0]),  # Numpy array specifying lower bounds
			ub=np.array(self.bounds[:, 1]),  # Numpy array specifying upper bounds
    			n_init=dim,  # Number of initial bounds from an Symmetric Latin hypercube design
    			...
    			n_training_steps=self.n_training_steps,  # Number of steps of ADAM to learn the hypers
...
for function in feeder:
	experiment.run(function, finish_line=1) #1000
